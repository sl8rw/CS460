...
void IRQ_handler()
{
    ...
    //TVALUE is extern defined as 0x1 in timer.c
    if (vicstatus & 0x0010){   // timer0,1=bit4
      if (*(tp->base+TVALUE)==0) // timer 0
         timer_handler();
    }
}
...
//	                                              proc.ksp
  ...
  PROC *pTemp=running->child;
  if(pTemp!=0)
  {
    printf("\nrunning pid's: %d->", pTemp->pid);
  }
  
  while(pTemp->sibling!=0)
  {
    printf("%d->", pTemp->sibling->pid);
    pTemp=pTemp->sibling;
  }
